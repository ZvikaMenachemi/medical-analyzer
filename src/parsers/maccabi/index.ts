...
/**
 * Parse Maccabi results from plain OCR text (no x/y positions available).
 * Each result row should have: Hebrew/English test name + numeric value + optional unit + optional range.
 */
function parseMaccabiOcrText(ocrText: string): ParsedResult[] {
  const results: ParsedResult[] = [];
  const lines = ocrText.split('\n').map(l => l.trim()).filter(l => l.length > 2);

  for (const line of lines) {
    // Skip header / metadata lines
    if (/תאריך|Date|ת\.ז|מכבי|Maccabi|שם\s*:|בדיקה\s*:|ערך\s*:|טווח\s*נורמה|מבוצע/i.test(line)) continue;
    if (!/\d/.test(line)) continue;

    // Extract range: two numbers separated by dash (e.g., "4.5-11.0" or "4.5 - 11.0")
    let rangeStr = '';
    const rangeMatch = line.match(/\b(\d+(?:[.,]\d+)?)\s*[-–]\s*(\d+(?:[.,]\d+)?)\b/);
    if (rangeMatch) {
      rangeStr = `${rangeMatch[1].replace(',', '.')}-${rangeMatch[2].replace(',', '.')}`;
    }

    // Find value in line with range removed (so range digits don't confuse value detection)
    const lineNoRange = rangeMatch ? line.replace(rangeMatch[0], ' ') : line;
    const valueMatch  = lineNoRange.match(/([<>]?\s*\d+(?:[.,]\d+)?)/);
    if (!valueMatch) continue;
    const valueStr = valueMatch[1].replace(',', '.').trim();

    // Skip ID numbers / years
    if (/^\d{5,}$/.test(valueStr)) continue;

    // Extract unit: letters immediately after the value number
    let unitStr = '';
    const unitMatch = lineNoRange.match(/\d+(?:[.,]\d+)?\s*([a-zA-Zµ%]+(?:\/[a-zA-Zµ%\d]+)?)/);
    if (unitMatch?.[1]) {
      const u = unitMatch[1];
      if (u.length <= 10 && !/^(the|and|or|in|of|to|is|at)$/i.test(u)) unitStr = u;
    }

    // Extract test name: prefer longest Hebrew word sequence; fall back to English
    let testName = '';
    const hebrewSeqs = line.match(/[\u05D0-\u05EA][\u05D0-\u05EA\s''"\-]{0,40}/g);
    if (hebrewSeqs) {
      testName = hebrewSeqs
        .map(s => s.trim())
        .filter(s => s.length >= 2)
    ...
    if (!testName) {
      const engMatch = line.match(/[A-Z][A-Za-z\s\-]{2,}/);
      if (engMatch) testName = engMatch[0].trim();
    }

    if (!testName || testName.length < 2) continue;
    // Skip unit-like names that leaked through
    if (/^(g|mg|ml|dl|ul|fl|pg|ng|ug|µg|iu|miu|nmol|pmol|mmol|µmol|units|K|L|M)\b/i.test(testName)) continue;
